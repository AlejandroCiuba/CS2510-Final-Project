...
def main(args: argparse.Namespace):
    ...
    commit_times = TIMERVAULT.to_dict()['recordings']['total'][:-1]

    # Create queries for TiKV metrics 
    # total_time = TIMERVAULT.to_dict()['recordings']['total'][-1]
    # cpu_usage_query = f'avg_over_time(process_cpu_seconds_total[{int(total_time)}s])'
    # memory_usage_query = f'avg_over_time(node_memory_MemTotal[{int(total_time)}s])'
    ...
    print(f"RESULT | AVG COMMIT TIME: {sum(commit_times) / len(commit_times)}")
